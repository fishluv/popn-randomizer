...
import { LEVELS, parseSranLevel, parseVersionFolder, SranLevel, SRAN_LEVELS, VERSION_FOLDERS } from 'popn-db-js';
import styles from './ControlPanel.module.scss';
import { ChartDisplayOptions } from './ChartDisplay';
import { ChartQuerySampleOptions } from '../pages/RandomizerApp';
import { ChartDrawOptions, ALL_VERSION_FOLDERS, versionFoldersToQueryValue, NONE_VERSION_FOLDERS } from './ChartDrawOptions';
...
export type ControlPanelState = ChartDrawOptions & Partial<ChartDisplayOptions> & ControlPanelOptions
...
export default class ControlPanel extends React.Component<ControlPanelProps, ControlPanelState> {
  ...
  onInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (id === 'displayGenreInput') {
      ...
      newState = {
        includeDiffs: checked ? `${includeDiffs}e` : includeDiffs!.replace("e", ""),
      };
      this.setState(newState);

    } else if (id === 'includeNormalInput') {
      newState = {
        includeDiffs: checked ? `${includeDiffs}n` : includeDiffs!.replace("n", ""),
      };
      this.setState(newState);

    } else if (id === 'includeHyperInput') {
      newState = {
        includeDiffs: checked ? `${includeDiffs}h` : includeDiffs!.replace("h", ""),
      };
      this.setState(newState);

    } else if (id === 'includeExInput') {
      newState = {
        includeDiffs: checked ? `${includeDiffs}x` : includeDiffs!.replace("x", ""),
      };
      this.setState(newState);

    } else if (id === 'onlyIncludeHardestInput') {
      newState = { hardestDiff: checked ? parseIncludeOption("only") : parseIncludeOption("include") };
      this.setState(newState);
      ...
      newState = { floorInfection: checked ? parseIncludeOption("exclude") : parseIncludeOption("include") };
      this.setState(newState);

    } else if (id === 'excludeBuggedBpmsInput') {
      newState = { buggedBpms: checked ? parseIncludeOption("exclude") : parseIncludeOption("include") };
      this.setState(newState);
  ...
  getSummaryString = () => {
    ...
    if (sranModeEnabled) {
      ...
      return `${count} songs, sran lv ${normSranLevel(sranLevelMin!)}~${normSranLevel(sranLevelMax!)}`;
    }
    ...
    return (
      <section className={rootClassName}>
        ...
        <section className={styles.buttonsAndSummary}>
          ...
          <button className={styles.clearButton} type="button" onClick={this.onClearClick}>
            🗑
          </button>

          <button className={styles.collapseButton} type="button" onClick={this.toggleCollapsed}>
        ...
        <ReactModal
          ...
          style={{ overlay: { zIndex: 10 }, content: { inset: 0, padding: "1rem" } }}
        >
          <button
          ...
          <section className={cx(styles.control, styles.includeDiffs)}>
            ...
            <input
              ...
              onChange={this.onInputChange}
            />
            <label htmlFor="includeAllDiffsInput">
              All
            </label>
            ...
            <input
              ...
              onChange={this.onInputChange}
            />
            <label htmlFor="includeChooseDiffsInput">
              Choose
            </label>
          ...
          <section className={cx(styles.control, styles.includeFolders)}>
            ...
            <input
              ...
              onChange={this.onInputChange}
            />
            <label htmlFor="includeAllFoldersInput">
              All
            </label>
            ...
            <input
              ...
              onChange={this.onInputChange}
            />
            <label htmlFor="includeChooseFoldersInput">
              Choose
            </label>
            ...
            {versionFoldersRadio === "choose" && (
              <section className={styles.inner}>
                {Array.from(Array(VERSION_FOLDERS.length).keys()).reverse().map(folderNum => (
                  <section className={styles.folderSelect} key={folderNum}>
                    ...
                    <label htmlFor={`folder${folderNum}Input`}>
                      <FolderPill songFolder={parseVersionFolder(String(folderNum))} style="normal" />
                    </label>
                  </section>
                ))}
              </section>
          ...
          <section className={cx(styles.control, styles.chartAppearance)}>
            ...
            <input
              ...
              onChange={this.onInputChange}
            />
            <label htmlFor="displayStyleNormalInput">
              Normal
            </label>
            ...
            <input
              ...
              onChange={this.onInputChange}
            />
            <label htmlFor="displayStyleCompactInput">
              Compact
            </label>

            {displayStyle === "compact" && (
              <section className={styles.compactInfo}>
                When compact, tap on charts to reveal their {preferGenre ? "genre" : "title"}
              </section>
            )}
          </section>
